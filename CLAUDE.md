...
new behaviour is a Python function called through `py()`/`enaPy()`, or plain JS.
`app/` holds the page (`app/static/`) and the Python it runs (`app/*.py`,
`app/pyodide/`). After editing `app/`, rebuild (`task serve`).
...
worker, `app/static/py/worker.js`) are stubbed in `test_ui.py` with
...
**Rule:** any change under `app/static/*` (tabs, DataHarmonizer panels, JS)
...
(`app/schema_service.py` `ROLE_FOLDERS`): sample→`mimicc`,
...
branch in `app/static/dataharmonizer.js` `initDhFrames()`.
